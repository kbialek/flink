...
public final class ConsulCompletedCheckpointStore implements CompletedCheckpointStore {
		...
		if (checkpointPaths != null) {
			...
			checkpointPaths.stream().map(path -> {
				try {
					...
					ByteArrayInputStream bais = new ByteArrayInputStream(binaryValue.getValue());
					ObjectInputStream ois = new ObjectInputStream(bais);
					return (CompletedCheckpoint) ois.readObject();
	...
	private void writeCheckpoint(CompletedCheckpoint checkpoint) throws IOException {
		String key = jobPath() + checkpoint.getCheckpointID();
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ObjectOutputStream oos = new ObjectOutputStream(baos);
		oos.writeObject(checkpoint);
		oos.close();
		client.setKVBinaryValue(key, baos.toByteArray());
	}
